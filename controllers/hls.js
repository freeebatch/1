...
const convertMPDToHLS = async (mpdId, quality, type) => {
    try {
        ...
        if (type === "download") {
            ...
            replacement = `https://1-six-jet.vercel.app/dash/${mpdId}/hls/${quality}/$1?Policy=${headers['Cookie'].match(/CloudFront-Policy=([^;]+)/)[1]}&Key-Pair-Id=${headers['Cookie'].match(/CloudFront-Key-Pair-Id=([^;]+)/)[1]}&Signature=${headers['Cookie'].match(/CloudFront-Signature=([^;]+)/)[1]}`;
        }
        
        let newText = main_data2
            .replace(pattern, replacement)
            .replace(/https:\/\/api\.penpencil\.(xyz|co)\/v1\/videos\//g, `https://studywithme-alpha.vercel.app/`);
    ...
    try {
        ...
        for (let i = 0; i < adaptationSets.length; i++) {
            ...
            for (let j = 0; j < representations.length; j++) {
                ...
                if(type === 'play'){
                    hlsPlaylist += `https://1-six-jet.vercel.app/hls?v=${mpdId}&quality=${quality}&type=play\n`;
                }else{
                    hlsPlaylist += `https://1-six-jet.vercel.app/hls?v=${mpdId}&quality=${quality}&type=download\n`;
                }
            }
        }
        return hlsPlaylist;
